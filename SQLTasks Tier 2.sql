...
SELECT f.name AS facility_name,
       CASE WHEN b.memid = 0 THEN 'Guest' ELSE CONCAT(m.firstname, ' ', m.surname) END AS member_name,
...
WHERE b.starttime >= '2012-09-14' AND b.starttime < '2012-09-15'
      AND (b.memid = 0 AND f.guestcost > 30 OR b.memid != 0 AND f.membercost > 30)
ORDER BY cost DESC;
...
JOIN Facilities AS f ON m.facid = f.facid
JOIN Bookings AS b ON m.facid = b.facid AND m.memid = b.memid
WHERE (b.memid = 0 AND f.guestcost > 30) OR (b.memid != 0 AND f.membercost > 30)
ORDER BY cost DESC;
